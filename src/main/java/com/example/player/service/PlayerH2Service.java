...
public class PlayerH2Service implements PlayerRepository{
    ...
    public Player updatePlayer(int playerId,Player player){
        if(player.getPlayerName() != null){
            db.update("update player set playerName = ? where playerId = ?",
                        player.getPlayerName(),playerId);
        }
        if(player.getJerseyNumber() != 0){
            db.update("update player set jerseyNumber = ? where playerId = ?",
                        player.getJerseyNumber(),playerId);
        }
        if(player.getRole() != null){
            db.update("update player set role = ? where playerId = ?",
    ...
    public void deletePlayer(int playerId){
        db.update("delete * from team where playerId  = ?");
    }
}
